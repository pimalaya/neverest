...
use email::{
    account::{
        ...
        discover::config::{AuthenticationType, SecurityType, ServerType},
    },
...
use crate::{
    ...
    ui::{prompt, THEME},
    wizard_log, wizard_prompt, wizard_warn,
};
...
static OAUTH2_MECHANISMS: &[&str] = &[XOAUTH2, OAUTHBEARER];
static XOAUTH2: &str = "XOAUTH2";
static OAUTHBEARER: &str = "OAUTHBEARER";

static SECRETS: &[&str] = &[KEYRING, RAW, CMD];
static KEYRING: &str = "Ask my password, then save it in my system's global keyring";
static RAW: &str = "Ask my password, then save it in the configuration file (not safe)";
static CMD: &str = "Ask me a shell command that exposes my password";
...
pub async fn configure(account_name: &str) -> Result<BackendConfig> {
    ...
    spinner.set_style(
        ProgressStyle::with_template("{spinner:.blue} Discovering IMAP config…")
            .unwrap()
            .tick_strings(&["⠋", "⠙", "⠹", "⠸", "⠼", "⠴", "⠦", "⠧", "⠇", "⠏"]),
    );

    let autoconfig_email = email.to_owned();
    let autoconfig = tokio::join!(
        // wait a minima 1 sec to prevent the spinner to disappear too
        // quickly
        tokio::time::sleep(Duration::from_secs(1)),
        email::account::discover::from_addr(&autoconfig_email),
    );
    let autoconfig = autoconfig.1.ok();
    ...
    if let Some(config) = autoconfig {
        if config.is_gmail() {
            ...
            wizard_warn!("https://pimalaya.org/neverest/cli/latest/configuration/gmail.html");
            println!();
        }
    }
