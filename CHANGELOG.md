...
- The configuration wizard asks for a single input, an email address, and derives the account name from its domain.
  ...
  Discovery runs every mechanism in parallel (fixed provider rules, PACC, Thunderbird Autoconfiguration, RFC 6186 SRV, RFC 6764 DAV) under a deadline rather than trying them in series, and every reachable service is proposed. Only backends compiled into the running build are offered, and only the SASL mechanisms both the server advertises and the configuration can express. Every configured connection is tested before the account is written. A bare `neverest` prints the help, and offers the wizard only when it finds no configuration, the way a bare `himalaya` does. A command that finds no configuration offers the wizard too, then fails naming the path it looked at when nothing landed; scripts and JSON callers skip the offer and get that failure straight away.

- `neverest configure` generates an account and never edits one.

  It reads the configuration already on disk for two things only, the account names it takes and whether one of them claims `default`, then appends the generated `[accounts.<name>]` table to it as plain text. Comments, ordering and hand-written formatting survive, which re-serializing the parsed document did not. The name is derived rather than asked for, and suffixed (`posteo-2`) until it is free, since two tables of one name make the whole file fail to parse; the new account claims `default` only when no other one does. `--json` or a redirected stdout prints the account and touches no file, so `neverest configure > config.toml` writes it itself, and declining the prompt prints it too, so a generated account is never lost. Changing an account already configured is a job for your editor, against [config.sample.toml](./config.sample.toml).
...
- An account syncing two endpoints reported nothing its servers had done, and dropped some of what it did itself.

  The two-endpoint reconcile read only conflicts out of its passes, so a flag a server had set and an item a server had dropped reached no report at all, where the one-endpoint path recovers both from the same per-item events. It could not have read them anyway: its opening pass pushed as well as pulled, and a pushing pass's `FlagsChanged` and `Vanished` also echo the writes the run has just made, which is what itemizing them naively would double-count. That push cost the other half of the report too, the cross-side plan being read from the projection after the pass, so any propagation the pass had already applied was itemized as nothing: against two Stalwart servers, a `\Flagged` set on one endpoint reported neither the change nor its crossing, and the run called itself quiet. The opening pass now pulls and nothing else, as the one-endpoint path always did, and the pushing passes follow it.
